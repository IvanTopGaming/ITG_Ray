...
func main() {
	...
	f, err := os.Create(out) //nolint:gosec // path is app-controlled, not attacker-supplied
	if err != nil {
		exit(err)
	}
...
func generate(pkgPath string, w io.Writer) error {
	...
	_, _ = fmt.Fprintln(w, "// Code generated by internal/bridge/codegen. DO NOT EDIT.")
	_, _ = fmt.Fprintln(w)
	for _, n := range typeOrder {
		_, _ = fmt.Fprintln(w, typeBodies[n])
	}
	_, _ = fmt.Fprintln(w, "export interface RpcMethods {")
	for _, e := range rpcEntries {
		_, _ = fmt.Fprintf(w, "  \"%s\": { params: %s; result: %s };\n", e.Method, e.ParamsTS, e.ResultTS)
	}
	_, _ = fmt.Fprintln(w, "}")
	_, _ = fmt.Fprintln(w)
	_, _ = fmt.Fprintln(w, "export type RpcMethod = keyof RpcMethods;")
	_, _ = fmt.Fprintln(w, "export type RpcParams<M extends RpcMethod> = RpcMethods[M][\"params\"];")
	_, _ = fmt.Fprintln(w, "export type RpcResult<M extends RpcMethod> = RpcMethods[M][\"result\"];")
	_, _ = fmt.Fprintln(w)
	_, _ = fmt.Fprintln(w, "export type EventTopic =")
