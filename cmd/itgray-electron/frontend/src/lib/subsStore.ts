...
export type SubsActions = {
  add: (url: string, userAgent?: string) => Promise<void>;
  edit: (id: string, url: string, userAgent?: string) => Promise<void>;
...
async function addAction(url: string, userAgent?: string): Promise<void> {
  setState({ ...state, inFlight: { ...state.inFlight, adding: true } });
  try {
    const view = await AddSub(url, userAgent ?? "");
...
async function editAction(id: string, url: string, userAgent?: string): Promise<void> {
  const editingNext = new Set(state.inFlight.editing);
  editingNext.add(id);
  setState({ ...state, inFlight: { ...state.inFlight, editing: editingNext } });
  try {
    const view = await EditSub(id, url, userAgent ?? "");
