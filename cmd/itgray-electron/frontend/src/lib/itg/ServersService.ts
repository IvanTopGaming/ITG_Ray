...
import type { ServersEditResult } from "../../../../src/shared/protocol";
...
export function Edit(id: string, uri: string, name: string): Promise<ServersEditResult> {
  return (svc().edit?.({ id, uri, name }) as Promise<ServersEditResult>)
    ?? Promise.reject(new Error("servers.edit is unavailable"));
}
